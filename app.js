...
async function proceedToPayment(e) {
  ...
  // ✅ Only confirm file upload, don’t show job code yet
...
// ===============================
// Upload Receipt (Show Job Code Here)
// ===============================
async function uploadReceipt(e) {
  ...
  // ✅ Show Job Code Popup with Copy + Close
  ...
  popup.innerHTML = `
    <p><strong>Your Job Code:</strong></p>
    <p style="font-size:18px; color:#003366; margin:10px 0;">${generatedJobCode}</p>
    <button id="copyJobCodeBtn" style="padding:8px 12px; border:none; background:#003366; color:#fff; border-radius:6px; cursor:pointer; margin-right:10px;">Copy Code</button>
  ...
  // Copy button
  document.getElementById("copyJobCodeBtn").addEventListener("click", () => {
    navigator.clipboard.writeText(generatedJobCode).then(() => {
